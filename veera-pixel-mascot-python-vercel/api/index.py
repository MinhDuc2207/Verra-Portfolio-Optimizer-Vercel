...
def get_fundamentals():
    ...
    for sym in symbols:
        try:
            if market == 'VN':
                # Try fetching indicators from TCBS API
                ...
                if res.status_code == 200:
                    ...
                    if isinstance(row, dict):
                        debt_on_asset = row.get("debtOnAsset") or row.get("debtToAsset") or row.get("debtAsset")
                        market_cap = row.get("marketCap") or row.get("marketCapitalization") or "N/A"
                        if debt_on_asset is not None:
                            lev = float(debt_on_asset)
                            leverage_val = lev / 100.0 if abs(lev) > 1 else lev
                            quality = "Partial Data"
                            
                # Fallback to database if TCBS returns N/A
                if leverage_val == "N/A" and os.path.exists(DB_PATH):
                    try:
                        conn = sqlite3.connect(DB_PATH)
                        df = pd.read_sql_query(f"SELECT leverage, expected_return as fcf FROM stock_metrics WHERE symbol = '{sym}' ORDER BY metric_date DESC LIMIT 1", conn)
                        conn.close()
                        if not df.empty:
                            leverage_val = float(df.iloc[0]['leverage']) if df.iloc[0]['leverage'] is not None else "N/A"
                            fcf_yield = float(df.iloc[0]['fcf']) if df.iloc[0]['fcf'] is not None else "N/A"
                            quality = "Partial Data"
                    except Exception:
                        pass
                        
                data[sym] = {
                    "altmanZ": {"value": "N/A", "reliability": "Data Not Available", "source": "TCBS_Public_API", "note": "A true Altman Z score needs full statement detail, so Veera does not fabricate this value."},
                    "beneishM": {"value": "N/A", "reliability": "Data Not Available", "source": "TCBS_Public_API", "note": "A true Beneish M score needs multi-year standardized statements, so Veera leaves this as unavailable."},
                    "fcf": {"value": fcf_yield, "reliability": "TCBS Indicator" if fcf_yield != "N/A" else "Data Not Available", "source": "TCBS_Public_API"},
                    "leverage": {"value": leverage_val, "reliability": "TCBS Indicator" if leverage_val != "N/A" else "Data Not Available", "source": "TCBS_Public_API"},
                ...
                mc = info.get("marketCap") or info.get("enterpriseValue") or 50000000000
                fcf_yield = float(fcf) / float(mc) if fcf and mc else "N/A"
                
                dte = info.get("debtToEquity")
                leverage_val = float(dte) / 100.0 if dte is not None else "N/A"
                
                data[sym] = {
                    "altmanZ": {"value": "N/A", "reliability": "Data Not Available", "source": "Yahoo_Finance_Open", "note": "True Altman Z needs full standardized statements."},
                    "beneishM": {"value": "N/A", "reliability": "Data Not Available", "source": "Yahoo_Finance_Open", "note": "True Beneish M needs multi-year accounting detail."},
                    ...
                    "marketCap": {"value": mc, "reliability": "Yahoo Fundamental Field", "source": "Yahoo_Finance_Open"},
